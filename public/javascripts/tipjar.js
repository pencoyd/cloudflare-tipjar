  ...
  function(config, $) {
    ...
    var initialize = function() {
      ...
      window.CF_TIPJAR['ELEMENTS']['tipjar_frame']
        .find("#hide_tipjar_frame").click(hideTipjar).end()
        .find("#cf_tipjar_start").click(startOAuth).end()

      window.CF_TIPJAR['ELEMENTS']['tipjar']
        .find("#tipjar_button").click(displayTipjarFrame).end()
        .find("#hide_tipjar").click(hideTipjar).end();
    ...
    var finishOAuth = function(event) {
      ...
      window.CF_TIPJAR['ELEMENTS']['tipjar_frame']
        .find('#copy_heading').html("Please authorize this transaction.").end()
        .find('#copy_body').html("You are tipping $" + window.CF_TIPJAR['TIP']).end()
        .find('#midsection').html('&nbsp;').end();
        .find("#endsection").html('<input id="cf_tipjar_pin" name="cf_tipjar_pin" placeholder="PIN" type="password" maxlength="4"><a class="button" onclick="sendPayment();">Done</a>').end()
    ...
    var congratulations = function() {
      window.CF_TIPJAR['ELEMENTS']['tipjar_frame']
        .find('#copy_heading').html("You're a good human.").end()
        .find('#copy_body').html("").end()
        .find('#midsection').html("You tipped the author $" + window.CF_TIPJAR['TIP']).end()
        .find('#endsection').html('<a class="button" onclick="hideTipjar();">Close</a>').end()
    }
    ...
    var sendPayment = function() {
      ...
      $.post(
        ...
        function(data) {
          ...
          window.CF_TIPJAR['ELEMENTS']['tipjar_frame'].find('#midsection').html("Please try again.");
        }
      );
